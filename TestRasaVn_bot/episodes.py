...
def episodes_run(agent, NLU, simulator, model, params, mapping, output='epochs.csv', use_model = True, thre_no_intent=0.5):
    ...
    for episode in range(num_episodes):
        ...
        repr = get_representation(question, map_state2index)
        ...
        if not use_model: #interactive rewards
            ...
            reward_int = -99
            reward_int = get_reward(question, action)
            reward = float(reward_int)
            
        else: # reward from score model 
            
            emb_u = emb_states[map_state2index[question]]
            emb_a = emb_actions[index_action]
        
            reward_model = get_reward_model([question], [action], [emb_u], [emb_a], model, map_action2answer)
            reward = reward_model
            ...
            if action == 'No intent detected':
                ...
                other_utt = [question]*(len(map_action2index)-1)
                other_act = list(map_action2index.keys())
                other_act.remove('No intent detected')         
                emb_u = [emb_states[map_state2index[question]]] *(len(map_action2index)-1)
...
def get_reward_model(questions, actions, emb_u, emb_a, model, map_action2answer):
    """ score model inference to get reward """
    
    answers = [map_action2answer.loc[map_action2answer['intent'] == action]['answer'].values[0] for action in actions]
    ...
    for iter in range(num_iter_warm):
        ...
        for episode in range(episode_max):
            ...
            if use_Q:
                ...
                intent = 'Unknown'
                action = map_index2action[index_action-1]
            else:
                action, intent, confidence = agent.rule_policy(question)
            ...
            if conv is not None:
                ...
                question = conv['question'] #can be different from 1 in slots
                s_t_plus1 = question
...
def testing(agent, mapping, Q_table, from_Q_table = True, verbose=True, num_test=-1):
    """ Test performance of DQN agent after warm-up training, by comparing answers with NLU answers"""
